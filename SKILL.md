...
- `--ready-stable-seconds` (default 2.0) debounces `ready_regex`: a match must hold continuously for that long before completing with `ready_matched`, so a stale indicator visible for one poll can't false-complete. `0` fires on first match. A custom Claude status line can key `ready_regex` off state indicators (`✓ done` / `⏸ waiting`) — see README "Detecting readiness from a custom Claude status line".
