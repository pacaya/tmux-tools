...
`capture [--lines N | --all]`: capture the target pane. With no line flag it captures the visible pane; `--lines N` captures the tail range; `--all` requests full scrollback history. A pane on the alternate screen has no tmux scrollback, so `--all` there returns only the visible screen: raw and concise say so on stderr, and JSON reports `scrollback_available: false`. Example: `tmux-tools capture --target build --lines 50`.
...
`wait-idle [--idle-seconds F] [--ready-stable-seconds F] [--timeout SEC] [--until REGEX] [--hint-lines N]`: wait until the resolved surface's `ready_regex` matches stably, `--until` matches, the visible capture is unchanged for `F` seconds, or timeout is reached. Quiet output completes the wait unless the surface's `busy_regex` classifies the pane as busy, in which case only a ready match, `--until`, or the timeout ends it. Defaults are `--idle-seconds 2.0`, `--ready-stable-seconds 2.0`, `--timeout 120.0`, and `--hint-lines 10`. `--ready-stable-seconds` is how long the agent's `ready_regex` must match continuously before completing (debounces a stale indicator that lingers for one poll); `0` fires on first match. On a concise timeout, the reason line includes `idle_for=<seconds>` and is followed by a delimited bottom-N non-blank-lines hint; `--hint-lines 0` disables the tail. Example: `tmux-tools wait-idle --target codex-helper --idle-seconds 3 --until "Done"`.

`prompt <TEXT> [--idle-seconds F] [--ready-stable-seconds F] [--timeout SEC] [--until REGEX]`: send text plus Enter, wait for completion, and return output from the pre-submission history mark (the agent's echo is included). It settles on the resolved surface's `ready_regex`, or by idle detection only when the surface supplies no patterns; with patterns present an unknown capture never settles as idle. It reports turn observation (whether `busy_regex` was seen between submission and settle) and completeness (no scrollback, or an evicting history) on stderr for raw/concise and as `turn_observed`/`complete` in JSON — both always stated, even for an empty result. Sessions `launch`/`spawn-agent` create get `history-limit` 50000 so a scrolled-off turn survives; for other sessions set it in `~/.tmux.conf` and expect truncation to be reported through completeness. Defaults are `--idle-seconds 2.0`, `--ready-stable-seconds 2.0`, and `--timeout 120.0`. Example: `tmux-tools prompt --target codex-helper "explain this file" --idle-seconds 3`.

`spawn-agent <AGENT> [--surface NAME] [--access PROFILE] [--resume ID] [--name NAME] [--cwd PATH] [--split h|v|window] [--size N] [--bare] [-- EXTRA_ARGS...]`: look up an agent in the registry, launch the selected named surface with the chosen access profile, resume identifier, and extra args, then register pane metadata. `--surface` picks a rendering; omitted, the agent's default surface is recorded as `@tt-surface`. `--resume ID` validates the identifier against the resolved surface's declared shape before creating any pane, then appends that surface's resume arguments; a malformed identifier, or a surface with no resume syntax, fails with a message and creates no pane, and the surface-supplied resume argument does not mark the pane surface-unvalidated. Trailing caller arguments mark the pane `@tt-surface-unvalidated`. Same default-split (30:70 horizontal) and `--split window` opt-out as `launch`. Same keep-open wrap as `launch` — if the agent crashes the pane survives as a plain shell so the failure output is preserved. Pass `--bare` to opt out. A session it creates gets `history-limit` 50000. Note: `@tt-agent` is set at launch and not cleared, so `list`'s `agent=` column reflects the original launch even after the agent has exited. Example: `tmux-tools spawn-agent codex --access read-only --name codex-helper --cwd .`.
...
`interrupt`: send the target pane's resolved surface `interrupt_key` (`C-c` when the pane has no surface). On a surface whose `quit_when_idle` is true, refuses unless the pane is positively busy, and refuses a surface-unvalidated pane outright; a surface declaring no quit hazard always sends its key. Same `--force` (self-pane) and `--any` (non-owned or cross-cwd) safety guards as `kill`. Example: `tmux-tools interrupt --target build`.
...
`capture` defaults to the visible pane; add `--all` when full scrollback history is required. A pane on the alternate screen has no tmux scrollback, so there `--all` returns only the visible screen: raw and concise announce the limit on stderr, and JSON reports `scrollback_available: false`.

## Completion-Heuristics Gotchas

- `--idle-seconds` is the fallback completion signal, not the dominant one: for `wait-idle`, a quiet pane means done only when the classifier returns unknown (no surface, no patterns, or an ambiguous capture); for `prompt`, idle detection applies only when the surface supplies no patterns or the pane resolves to no surface, so with patterns present an unknown capture never settles as idle. It never overrides a `busy_regex` classification — a busy pane ends only at a ready match, `--until`, or the timeout. If the pane prints heartbeat lines, increase `--idle-seconds`.
...
- Built-in registry `ready_regex` values live on each agent's single/default surface: `cursor = "→ (Add a follow-up|Plan, search, build anything)\\s*$"` (scanned against the bottom 4 non-blank lines via `ready_lines = 4`) and `agy = "\\? for shortcuts"`. `claude`'s `rich` and `flat` surfaces and `codex`'s `default` surface ship **no** `ready_regex` (their native chrome can't discriminate idle from busy) and fall back to idle/`--until`; key codex readiness off its status line (`· Ready · Context` vs `· Working ·`) via a user `agents.toml` or an explicit `--until '· Ready · Context'`.
...
- A concise timeout starts with `reason=timed_out duration=<seconds> idle_for=<seconds>` and then the delimited bottom `--hint-lines` non-blank lines. Read that cheap hint between wait windows; capture the full settled result once at the terminator. JSON exposes numeric `idle_for` and leaves `final_capture` unchanged.
